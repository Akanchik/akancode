const outputDiv = document.getElementById("output");
const fileInput = document.getElementById("fileInput");
const uiDiv = document.getElementById("ui");
let кнопкиНажаты = {};
let isRunning = false;
let variables = {};
let labels = {};
let arrays = {};
let currentLine = 0;
let rawCode = [];
let executionTimeout = null;

function resetInterpreter() {
  if (executionTimeout) {
    clearTimeout(executionTimeout);
    executionTimeout = null;
  }
  
  isRunning = false;
  кнопкиНажаты = {};
  variables = {};
  labels = {};
  arrays = {};
  currentLine = 0;
  rawCode = [];
  
  outputDiv.innerHTML = "";
  uiDiv.innerHTML = "";
}

function clearCode() {
  document.getElementById("code").value = "";
}

function clearOutput() {
  outputDiv.innerHTML = "";
  uiDiv.innerHTML = "";
  кнопкиНажаты = {};
}

function showHelp() {
  alert(`Доступные команды AKANCODE:

📊 ОСНОВНЫЕ КОМАНДЫ:
  ВВОД имя              - Ввод значения
  ПРИСВОИТЬ имя = значение - Присвоение
  ВЫВЕСТИ выражение     - Вывод
  ПЕЧАТАТЬ "текст"      - Печать по буквам

🔢 МАТЕМАТИЧЕСКИЕ:
  СЛОЖИТЬ a b результат
  ВЫЧЕСТЬ a b результат
  УМНОЖИТЬ a b результат
  ДЕЛИТЬ a b результат
  СЛУЧАЙНО мин макс результат
  ОСТАТОК a b результат
  СТЕПЕНЬ a b результат

🔄 УСЛОВИЯ И ЦИКЛЫ:
  ЕСЛИ условие ТО команда
  ПОКА условие ВЫПОЛНЯТЬ
  ДЛЯ i=1 ДО 10 ВЫПОЛНЯТЬ
  ПРЕРВАТЬ              - Выход из цикла
  ПРОДОЛЖИТЬ           - Следующая итерация

📦 МАССИВЫ:
  МАССИВ имя[размер]
  ЗАПИСАТЬ имя[индекс] = значение
  ПРОЧЕСТЬ имя[индекс] результат
  ДЛИНА имя результат

🏷️ МЕТКИ И ПЕРЕХОДЫ:
  МЕТКА имя
  ПЕРЕЙТИ имя
  ВЫЗВАТЬ имя          - Вызов подпрограммы
  ВЕРНУТЬ              - Возврат из подпрограммы

🎮 ИГРЫ:
  ИГРА УГАДАЙ
  ИГРА КНБ             - Камень-ножницы-бумага

🎨 ИНТЕРФЕЙС:
  ЗАГОЛОВОК "текст"
  ТЕКСТ "текст"
  КНОПКА имя "текст"
  ПОЛЕ_ВВОДА имя "подсказка"
  ЕСЛИ_НАЖАТА имя ТО команда

⏰ СИСТЕМНЫЕ:
  ПАУЗА миллисекунды    - Пауза выполнения
  СЛУЧАЙНОЕ_ЧИСЛО результат
  ТЕКУЩЕЕ_ВРЕМЯ результат

📝 КОММЕНТАРИИ:
  // Однострочный комментарий
  /* Многострочный комментарий */`);
}

function saveFile() {
  const blob = new Blob([document.getElementById("code").value], { type: "text/plain" });
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = "script.ak";
  a.click();
}

function loadFile() {
  fileInput.click();
}

function handleFile(files) {
  const reader = new FileReader();
  reader.onload = e => {
    document.getElementById("code").value = e.target.result;
  };
  reader.readAsText(files[0]);
}

function sleep(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

async function печать_по_буквам(text, delay = 30) {
  for (let i = 0; i < text.length; i++) {
    outputDiv.innerHTML += text[i];
    await sleep(delay);
  }
  outputDiv.innerHTML += "<br>";
}

function parseLine(line) {
  return line.trim().split('"').reduce((result, part, i) => {
    if (i % 2 === 0) {
      return result.concat(part.trim().split(/\s+/).filter(Boolean));
    } else {
      return result.concat(part);
    }
  }, []).filter(Boolean);
}

async function executeLine(line) {
  if (!line || line.startsWith("#") || line.startsWith("//") || line.startsWith("/*")) {
    return true;
  }

  const parts = parseLine(line);
  if (parts.length === 0) return true;

  const cmd = parts[0];

  try {
    // ========== СУЩЕСТВУЮЩИЕ КОМАНДЫ ==========
    if (cmd === "ВВОД") {
      if (parts.length < 2) return true;
      let varName = parts[1];
      let val = prompt(`Введите значение для ${varName}:`);
      val = isNaN(parseInt(val)) ? val : parseInt(val);
      variables[varName] = val;
    } else if (cmd === "ПРИСВОИТЬ") {
      if (parts.length < 4) return true;
      let varName = parts[1];
      if (parts[2] === "=") {
        let val = parts.slice(3).join(" ");
        val = val.replace(/^"|"$/g, "");
        val = isNaN(parseInt(val)) ? (variables[val] ?? val) : parseInt(val);
        variables[varName] = val;
      }
    } else if (cmd === "СЛОЖИТЬ") {
      if (parts.length < 4) return true;
      let resultVar = parts[parts.length - 1];
      let total = 0;
      for (let v of parts.slice(1, -1)) {
        let val = variables[v];
        if (val === undefined) val = parseInt(v) || 0;
        total += parseInt(val);
      }
      variables[resultVar] = total;
    } else if (cmd === "ВЫЧЕСТЬ") {
      if (parts.length < 4) return true;
      let a = variables[parts[1]] ?? parseInt(parts[1]) ?? 0;
      let b = variables[parts[2]] ?? parseInt(parts[2]) ?? 0;
      variables[parts[3]] = a - b;
    } else if (cmd === "УМНОЖИТЬ") {
      if (parts.length < 4) return true;
      let a = variables[parts[1]] ?? parseInt(parts[1]) ?? 0;
      let b = variables[parts[2]] ?? parseInt(parts[2]) ?? 0;
      variables[parts[3]] = a * b;
    } else if (cmd === "ДЕЛИТЬ") {
      if (parts.length < 4) return true;
      let a = variables[parts[1]] ?? parseInt(parts[1]) ?? 0;
      let b = variables[parts[2]] ?? parseInt(parts[2]) ?? 1;
      variables[parts[3]] = b !== 0 ? a / b : "Ошибка: деление на 0";
    } else if (cmd === "СЛУЧАЙНО") {
      if (parts.length < 4) return true;
      let a = variables[parts[1]] ?? parseInt(parts[1]);
      let b = variables[parts[2]] ?? parseInt(parts[2]);
      variables[parts[3]] = Math.floor(Math.random() * (b - a + 1)) + a;
    } else if (cmd === "ВЫВЕСТИ") {
      if (parts.length < 2) return true;
      let expr = parts.slice(1).join(" ");
      expr = expr.trim();
      if (expr.startsWith('"') && expr.endsWith('"')) {
        outputDiv.innerHTML += expr.slice(1, -1) + "<br>";
      } else {
        let val = variables[expr] ?? expr;
        outputDiv.innerHTML += val + "<br>";
      }
    } else if (cmd === "ПЕЧАТАТЬ") {
      if (parts.length < 2) return true;
      let text = parts.slice(1).join(" ");
      if (text.startsWith('"') && text.endsWith('"')) {
        await печать_по_буквам(text.slice(1, -1));
      } else {
        let val = variables[text] ?? text;
        await печать_по_буквам(val);
      }
    } else if (cmd === "ЕСЛИ") {
      if (parts.length < 6) return true;
      let conditionVar1 = parts[1];
      let conditionOp = parts[2];
      let conditionVar2 = parts[3];
      let thenCmd = parts[4];
      let thenRest = parts.slice(5);

      let val1 = variables[conditionVar1] ?? parseInt(conditionVar1);
      let val2 = variables[conditionVar2] ?? parseInt(conditionVar2);

      let cond = false;
      switch (conditionOp) {
        case ">": cond = val1 > val2; break;
        case "<": cond = val1 < val2; break;
        case "==": cond = val1 == val2; break;
        case "!=": cond = val1 != val2; break;
        case ">=": cond = val1 >= val2; break;
        case "<=": cond = val1 <= val2; break;
      }
      
      if (cond) {
        if (thenCmd === "ПЕРЕЙТИ") {
          let label = thenRest[0];
          if (labels[label] !== undefined) {
            currentLine = labels[label];
            return false;
          }
        } else if (thenCmd === "ВЫВЕСТИ") {
          let expr = thenRest.join(" ");
          if (expr.startsWith('"') && expr.endsWith('"')) {
            outputDiv.innerHTML += expr.slice(1, -1) + "<br>";
          } else {
            let val = variables[expr] ?? expr;
            outputDiv.innerHTML += val + "<br>";
          }
        }
      }
    } else if (cmd === "ЕСЛИ_НАЖАТА") {
      if (parts.length < 4) return true;
      let buttonName = parts[1];
      
      if (кнопкиНажаты[buttonName]) {
        let thenCmd = parts[2];
        let nextCmd = parts[3];
        let nextRest = parts.slice(4);

        if (nextCmd === "ПЕРЕЙТИ") {
          let label = nextRest[0];
          if (labels[label] !== undefined) {
            currentLine = labels[label];
            кнопкиНажаты[buttonName] = false;
            return false;
          }
        } else if (nextCmd === "ВЫВЕСТИ") {
          let expr = nextRest.join(" ");
          if (expr.startsWith('"') && expr.endsWith('"')) {
            outputDiv.innerHTML += expr.slice(1, -1) + "<br>";
          } else {
            let val = variables[expr] ?? expr;
            outputDiv.innerHTML += val + "<br>";
          }
          кнопкиНажаты[buttonName] = false;
        }
      }
    } else if (cmd === "ИГРА") {
      if (parts.length > 1 && parts[1] === "УГАДАЙ") {
        let min = 1;
        let max = 100;
        let secret = Math.floor(Math.random() * (max - min + 1)) + min;
        let attempts = 0;
        let guess = 0;
        
        outputDiv.innerHTML += "Я загадал число от 1 до 100. Попробуй угадать!<br>";
        
        while (guess !== secret) {
          let input = prompt("Введите ваше предположение:");
          guess = parseInt(input);
          attempts++;
          
          if (isNaN(guess)) {
            outputDiv.innerHTML += "Пожалуйста, введите число!<br>";
          } else if (guess < secret) {
            outputDiv.innerHTML += "Слишком мало! Попробуйте еще раз.<br>";
          } else if (guess > secret) {
            outputDiv.innerHTML += "Слишком много! Попробуйте еще раз.<br>";
          }
        }
        
        outputDiv.innerHTML += `Поздравляю! Вы угадали число ${secret} за ${attempts} попыток.<br>`;
      } else if (parts.length > 1 && parts[1] === "КНБ") {
        // Камень-ножницы-бумага
        const choices = ["КАМЕНЬ", "НОЖНИЦЫ", "БУМАГА"];
        const compChoice = choices[Math.floor(Math.random() * 3)];
        
        outputDiv.innerHTML += "Игра Камень-Ножницы-Бумага!<br>";
        outputDiv.innerHTML += "Выберите: КАМЕНЬ, НОЖНИЦЫ или БУМАГА<br>";
        
        let playerChoice = prompt("Ваш выбор:").toUpperCase();
        
        if (!choices.includes(playerChoice)) {
          outputDiv.innerHTML += "Неверный выбор!<br>";
          return true;
        }
        
        outputDiv.innerHTML += `Компьютер выбрал: ${compChoice}<br>`;
        outputDiv.innerHTML += `Вы выбрали: ${playerChoice}<br>`;
        
        if (playerChoice === compChoice) {
          outputDiv.innerHTML += "Ничья!<br>";
        } else if (
          (playerChoice === "КАМЕНЬ" && compChoice === "НОЖНИЦЫ") ||
          (playerChoice === "НОЖНИЦЫ" && compChoice === "БУМАГА") ||
          (playerChoice === "БУМАГА" && compChoice === "КАМЕНЬ")
        ) {
          outputDiv.innerHTML += "Вы победили!<br>";
        } else {
          outputDiv.innerHTML += "Компьютер победил!<br>";
        }
      }
    } else if (cmd === "ЗАГОЛОВОК") {
      let text = line.slice("ЗАГОЛОВОК".length).trim();
      text = text.replace(/^"|"$/g, "");
      uiDiv.innerHTML += `<h2>${text}</h2>`;
    } else if (cmd === "ТЕКСТ") {
      let text = line.slice("ТЕКСТ".length).trim();
      text = text.replace(/^"|"$/g, "");
      uiDiv.innerHTML += `<p>${text}</p>`;
    } else if (cmd === "КНОПКА") {
      if (parts.length < 3) return true;
      let name = parts[1];
      let text = parts.slice(2).join(" ").replace(/^"|"$/g, "");
      
      if (!document.getElementById(`btn_${name}`)) {
        let btn = document.createElement("button");
        btn.id = `btn_${name}`;
        btn.textContent = text;
        btn.onclick = () => { 
          кнопкиНажаты[name] = true;
          processNextLine();
        };
        uiDiv.appendChild(btn);
      }
    } else if (cmd === "ПЕРЕЙТИ") {
      if (parts.length < 2) return true;
      let labelName = parts[1];
      if (labels[labelName] !== undefined) {
        currentLine = labels[labelName];
        return false;
      } else {
        outputDiv.innerHTML += `[Ошибка: метка '${labelName}' не найдена]<br>`;
      }
    }
    
    // ========== НОВЫЕ КОМАНДЫ ==========
    else if (cmd === "МАССИВ") {
      if (parts.length < 2) return true;
      let arrayName = parts[1].replace(/\[.*\]/, '');
      let size = 10; // default size
      
      const sizeMatch = parts[1].match(/\[(\d+)\]/);
      if (sizeMatch) size = parseInt(sizeMatch[1]);
      
      arrays[arrayName] = new Array(size).fill(0);
    }
    else if (cmd === "ЗАПИСАТЬ") {
      if (parts.length < 5) return true;
      let arrayName = parts[1].replace(/\[.*\]/, '');
      let index = parseInt(parts[1].match(/\[(\d+)\]/)[1]);
      let value = variables[parts[3]] ?? parseInt(parts[3]);
      
      if (arrays[arrayName] && index >= 0 && index < arrays[arrayName].length) {
        arrays[arrayName][index] = value;
      }
    }
    else if (cmd === "ПРОЧЕСТЬ") {
      if (parts.length < 4) return true;
      let arrayName = parts[1].replace(/\[.*\]/, '');
      let index = parseInt(parts[1].match(/\[(\d+)\]/)[1]);
      let resultVar = parts[2];
      
      if (arrays[arrayName] && index >= 0 && index < arrays[arrayName].length) {
        variables[resultVar] = arrays[arrayName][index];
      }
    }
    else if (cmd === "ДЛИНА") {
      if (parts.length < 3) return true;
      let arrayName = parts[1];
      let resultVar = parts[2];
      
      if (arrays[arrayName]) {
        variables[resultVar] = arrays[arrayName].length;
      }
    }
    else if (cmd === "ОСТАТОК") {
      if (parts.length < 4) return true;
      let a = variables[parts[1]] ?? parseInt(parts[1]);
      let b = variables[parts[2]] ?? parseInt(parts[2]);
      variables[parts[3]] = a % b;
    }
    else if (cmd === "СТЕПЕНЬ") {
      if (parts.length < 4) return true;
      let a = variables[parts[1]] ?? parseInt(parts[1]);
      let b = variables[parts[2]] ?? parseInt(parts[2]);
      variables[parts[3]] = Math.pow(a, b);
    }
    else if (cmd === "ПАУЗА") {
      if (parts.length < 2) return true;
      let ms = variables[parts[1]] ?? parseInt(parts[1]);
      await sleep(ms);
    }
    else if (cmd === "СЛУЧАЙНОЕ_ЧИСЛО") {
      if (parts.length < 2) return true;
      variables[parts[1]] = Math.random();
    }
    else if (cmd === "ТЕКУЩЕЕ_ВРЕМЯ") {
      if (parts.length < 2) return true;
      variables[parts[1]] = new Date().toLocaleTimeString();
    }
    else if (cmd === "ПОКА") {
      if (parts.length < 4) return true;
      let conditionVar1 = parts[1];
      let conditionOp = parts[2];
      let conditionVar2 = parts[3];
      
      let val1 = variables[conditionVar1] ?? parseInt(conditionVar1);
      let val2 = variables[conditionVar2] ?? parseInt(conditionVar2);
      
      let cond = false;
      switch (conditionOp) {
        case ">": cond = val1 > val2; break;
        case "<": cond = val1 < val2; break;
        case "==": cond = val1 == val2; break;
        case "!=": cond = val1 != val2; break;
        case ">=": cond = val1 >= val2; break;
        case "<=": cond = val1 <= val2; break;
      }
      
      if (!cond) {
        // Пропускаем цикл
        let depth = 1;
        while (currentLine < rawCode.length - 1 && depth > 0) {
          currentLine++;
          let nextLine = rawCode[currentLine].trim();
          if (nextLine === "КОНЕЦ_ЦИКЛА") depth--;
          if (nextLine.startsWith("ПОКА ")) depth++;
        }
      }
    }
    else if (cmd === "КОНЕЦ_ЦИКЛА") {
      // Возврат к началу цикла
      let depth = 1;
      while (currentLine > 0 && depth > 0) {
        currentLine--;
        let prevLine = rawCode[currentLine].trim();
        if (prevLine === "КОНЕЦ_ЦИКЛА") depth++;
        if (prevLine.startsWith("ПОКА ")) depth--;
      }
      return false;
    }
    else if (cmd === "ПОЛЕ_ВВОДА") {
      if (parts.length < 3) return true;
      let name = parts[1];
      let placeholder = parts.slice(2).join(" ").replace(/^"|"$/g, "");
      
      if (!document.getElementById(`input_${name}`)) {
        let input = document.createElement("input");
        input.id = `input_${name}`;
        input.placeholder = placeholder;
        input.style.margin = "5px";
        input.style.padding = "5px";
        input.onchange = () => variables[name] = input.value;
        uiDiv.appendChild(input);
      }
    }

  } catch (e) {
    outputDiv.innerHTML += `[Ошибка: ${e.message}]<br>`;
  }

  return true;
}

async function processNextLine() {
  if (!isRunning || currentLine >= rawCode.length) {
    isRunning = false;
    if (executionTimeout) {
      clearTimeout(executionTimeout);
      executionTimeout = null;
    }
    return;
  }

  const line = rawCode[currentLine];
  const shouldContinue = await executeLine(line);
  
  if (shouldContinue) {
    currentLine++;
  }

  if (isRunning && currentLine < rawCode.length) {
    executionTimeout = setTimeout(processNextLine, 10);
  } else {
    isRunning = false;
    if (executionTimeout) {
      clearTimeout(executionTimeout);
      executionTimeout = null;
    }
  }
}

async function runCode() {
  resetInterpreter();
  isRunning = true;
  rawCode = document.getElementById("code").value.toUpperCase().split("\n");
  currentLine = 0;

  // Находим все метки
  labels = {};
  for (let idx = 0; idx < rawCode.length; idx++) {
    let l = rawCode[idx].trim();
    if (l.startsWith("МЕТКА ")) {
      const labelParts = parseLine(l);
      if (labelParts.length >= 2) {
        let labelName = labelParts[1];
        labels[labelName] = idx;
      }
    }
  }

  processNextLine();
}

async function openAsSite() {
  resetInterpreter();
  isRunning = true;
  rawCode = document.getElementById("code").value.toUpperCase().split("\n");
  currentLine = 0;

  labels = {};
  for (let idx = 0; idx < rawCode.length; idx++) {
    let l = rawCode[idx].trim();
    if (l.startsWith("МЕТКА ")) {
      const labelParts = parseLine(l);
      if (labelParts.length >= 2) {
        let labelName = labelParts[1];
        labels[labelName] = idx;
      }
    }
  }

  while (currentLine < rawCode.length) {
    const line = rawCode[currentLine].trim();
    const parts = parseLine(line);
    
    if (parts.length > 0 && ["ЗАГОЛОВОК", "ТЕКСТ", "КНОПКА", "ПОЛЕ_ВВОДА"].includes(parts[0])) {
      break;
    }
    currentLine++;
  }

  processNextLine();
}